...
public final class SettingsActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        root.setOrientation(LinearLayout.VERTICAL); root.setPadding(dp(20), dp(22), dp(20), dp(40)); root.setBackgroundColor(Color.rgb(247,248,251));
        scroll.addView(root);

        TextView title = text("PCBoard settings", 28, Color.rgb(20,23,30)); title.setPadding(0,0,0,dp(12)); root.addView(title);
        root.addView(text("Typing", 19, Color.rgb(55,80,170)));
        ...
        addSwitch(root, "Number row", KeyboardPreferences.NUMBER_ROW, false);
        ...
        addSeek(root, "Keyboard height", KeyboardPreferences.HEIGHT, 80, 130, preferences.heightPercent(), "%");
        addSeek(root, "Long-press delay", KeyboardPreferences.LONG_PRESS_DELAY, 250, 700, preferences.longPressDelay(), " ms");
        ...
        root.addView(button("Clear learned words", v -> {
            new PersonalLexicon(this).clear();
            Toast.makeText(this, "Learned words cleared", Toast.LENGTH_SHORT).show();
        }));
        root.addView(button("Clear clipboard history", v -> {
            new ClipboardRepository(this).clearAll();
    ...
    private void addSpinner(LinearLayout root, String label, String key, String[] values, String selected) {
        ...
        row.addView(spinner, new LinearLayout.LayoutParams(dp(180), dp(52))); root.addView(row);
    }
    ...
    private TextView section(String text) {
        TextView view = text(text, 19, Color.rgb(55,80,170)); view.setPadding(0, dp(22), 0, dp(4)); return view;
    }

    private Button button(String label, View.OnClickListener listener) {
        Button button = new Button(this); button.setText(label); button.setAllCaps(false); button.setOnClickListener(listener); return button;
    }
